(function (root) {
  ...
  function renderRoster(container, state, handlers) {
    ...
    state.tutors.forEach(function (t) {
      ...
      info.innerHTML =
        ...
        (U.ownRoom(t) ? ' · in ' + esc(U.ownRoom(t)) : '') +
        '</div>' +
        (t.email
   ...
   * that shift instead of adding one -- which is also the only way to hand a
   * shift to another tutor, since a drag cannot. */
  ...
  function openTutorDialog(tutor, settings, onSave) {
    ...
    dialog.innerHTML =
      ...
      '<div class="dialog__body">' +
        ...
        '<div class="field"><label for="f-room">Room, if not ' + esc(settings.location || 'the tutoring room') +
          '</label>' +
          '<input type="text" id="f-room" value="' + esc(working.room) + '" placeholder="Office 135-A">' +
          '<p class="field__hint">For a faculty member holding office hours in their own office. ' +
          'Their hours are drawn on the schedule with this room on them and count as cover ' +
          'for their classes, but not toward the limit on tutors in ' +
          esc(settings.location || 'the tutoring room') + '. Leave blank for everyone else.</p></div>' +
    ...
    dialog.querySelector('#btn-save-tutor').addEventListener('click', function () {
      ...
      var payload = {
        ...
        room: dialog.querySelector('#f-room').value.trim(),
