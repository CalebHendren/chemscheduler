(function (root) {
  ...
  function checkPlacement(state, assignment, day, start, end) {
    ...
    if (end - start < s.minShiftSlots && !U.ownRoom(tutor)) {
    ...
    // property of the tutoring room, so office hours held in a room of their
    // own clear it without being measured against it.
    if (U.ownRoom(tutor)) return { ok: true, overCapacity: false, overSlots: 0 };
    ...
    placed.endSlot = end;
    var cap = U.capacity(s, others.concat([placed]), state.tutors);
    var over = 0;
  ...
  function render(container, state, options) {
    ...
    var cap = U.capacity(state.settings, state.assignments, state.tutors);
  ...
  function buildBlock(a, state, labels, place, rh, dark, win) {
    ...
    node.innerHTML =
      ...
      (a.locked ? '' :
        ...
        '">×</button>') +
      // Which tutor works a shift is not something a drag can change, so the
      ...
      (room ? '<span class="block__room">' + TS.tutors.esc(room) + '</span>' : '') +
