(function (root) {
  ...
  function buildContext(state) {
    ...
    var tutors = state.tutors.map(function (t, i) {
      return {
        ...
        // Office hours in a room of their own: cover, but not a seat in the
        // tutoring room, and not held to its minimum shift length.
        seated: !U.ownRoom(t),
        minShift: U.ownRoom(t) ? 1 : Math.max(1, s.minShiftSlots || 1)
      };
    });
  ...
  function createSolution(ctx) {
    ...
    for (i = 0; i < ctx.tutors.length; i++) {
      ...
      sol.returnPenalty.push(emptyWeek().map(function () { return 0; }));
    }
    return sol;
  }
  ...
  // How many of the tutors in a half hour take a seat in the tutoring room.
  function seatedCount(ctx, list) {
    var n = 0;
    for (var k = 0; k < list.length; k++) if (ctx.tutors[list[k]].seated) n++;
    return n;
  }
  ...
  function overCapCount(sol, day) {
    var n = 0;
    for (var s = 0; s < U.SLOTS_PER_DAY; s++) {
      if (seatedCount(sol.ctx, sol.slotTutors[U.idx(day, s)]) > limitAt(sol, day, s)) n++;
    }
    return n;
  }
  ...
  function canAdd(sol, tutorIndex, day, start, end) {
    ...
    var len = end - start;
    if (len < t.minShift) return false;
    ...
    for (s = start; s < end && t.seated; s++) {
      var list = sol.slotTutors[U.idx(day, s)];
      var seated = seatedCount(ctx, list);
      if (s < ctx.caps.cutoff) {
        if (seated >= ctx.caps.day) return false;
        continue;
      }
      ...
      if (seated + 1 > U.capLimit(ctx.caps, s, carrying)) return false;
    }
  ...
  /* slotTutors is everyone covering a half hour, office hours included;
   * centerMap is only the seats taken in the tutoring room, which is what the
   * evening cap's carry-through reads.
   */
  function applyAdd(sol, block) {
    ...
    for (var s = block.start; s < block.end; s++) {
      ...
      sol.coreScore += slotScore(ctx, sol.slotTutors[i]);
      if (ctx.tutors[block.tutorIndex].seated) sol.centerMap[block.tutorIndex][block.day][s] = 1;
      row[s] = 1;
    }
    ...
    return {
      ...
      result: function () {
        return mergeAdjacent(best || []).map(function (b) {
          // A fixed block comes back out as the user wrote it -- the solver
          // only ever held it in place.
  ...
  function stats(state, assignments) {
    ...
    var cap = U.capacity(state.settings, assignments, state.tutors);
    var overSlots = 0;
  ...
  function validate(state, assignments) {
    ...
    assignments.forEach(function (a) {
      ...
      if (a.endSlot - a.startSlot < t.minShift) {
