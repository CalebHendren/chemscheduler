(function (root) {
  ...
  function drawTutorBlocks(pdf, state, labels, g) {
    g.days.forEach(function (day, col) {
      ...
      dayBlocks.forEach(function (a) {
        ...
        var textX = bx + 6;
        var textW = bw - 8;
        // Office hours say where they are held; a block too short for a line
        // of its own carries the room beside its time instead.
        var room = U.ownRoom(tutor);
        var tall = bh > 30;
        var roomLine = !!room && bh > 39;
        ...
        if (bh > 22) {
          ...
          var when = fitRange(pdf, a.startSlot, a.endSlot, textW);
          if (room && !roomLine) {
            when = fitText(pdf, U.formatRange(a.startSlot, a.endSlot) + ' · ' + room, textW, SMALL_PT);
          }
          pdf.text(when, textX, by + 17.5);
        }
        if (tall) {
          ...
          if (roomLine) {
            ...
            pdf.text(fitText(pdf, room, textW, SMALL_PT), textX, by + 34.5);
          }
        }
      });
    });
  ...
  function drawClassBlocks(pdf, state, labels, g) {
    ...
    g.days.forEach(function (day, col) {
      ...
      dayRuns.forEach(function (run) {
        ...
        // ORGO fitting and ORGO being cut to an ellipsis.
        var textX = bx + 5;
        var textW = bw - 6;
        pdf.setTextColor(INK[0], INK[1], INK[2]);
        pdf.setFont('helvetica', 'bold');
  ...
  /* One column while the roster is short enough to read down, two after. A
   * column is as wide as the space beside the QR block allows, so a class
   * list and a room both fit beside the name. */
  function drawTutorLegend(pdf, state, labels, legendX, legendY, width) {
    var cols = state.tutors.length <= 5 ? 1 : 2;
    var rows = Math.ceil(state.tutors.length / cols);
    var colW = width / cols;
    pdf.setFontSize(8);
    ...
    state.tutors.forEach(function (t, i) {
      var lx = legendX + Math.floor(i / rows) * colW;
      var ly = legendY + (i % rows) * 13;
      ...
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(MUTED[0], MUTED[1], MUTED[2]);
      var what = (U.maskToShort(U.subjectMask(t.subjects)).join(' · ') || '—') +
        (U.ownRoom(t) ? ', in ' + U.ownRoom(t) : '');
      pdf.text(truncate(pdf, what, colW - 30 - nameW), lx + 28 + nameW, ly);
    });
  }
  ...
  function drawClassLegend(pdf, state, legendX, legendY) {
    ...
    lanes.forEach(function (lane, i) {
      ...
      var ly = legendY + rowI * 13;
      drawSwatch(pdf, U.laneColors(lane, false), lx, ly);

      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(INK[0], INK[1], INK[2]);
      pdf.text(truncate(pdf, U.laneLabel(lane), 92), lx + 20, ly);
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(MUTED[0], MUTED[1], MUTED[2]);
      pdf.text(truncate(pdf, U.hoursLabel(totals[i]) + ' a week', 56), lx + 20 + 97, ly);
    });
  }
    ...
    for (var dl = 0; dl < U.DAYS; dl++) {
      ...
      blocks.forEach(function (a) {
        ...
        pdf.text(
          ...
          (U.ownRoom(tutor) ? ' (in ' + U.ownRoom(tutor) + ')' : ''),
          margin + 8, y2
        );
      });
    }
  ...
  function build(state) {
    ...
    pdf.setProperties({
      ...
      creator: 'Chatt State Chemistry Tutor Scheduler ' + U.VERSION
    });
  ...
  function drawCalendarPage(pdf, state, labels, pageW, pageH, margin, byClass) {
    ...
    var legendX = margin + 232;
    var legendY = footY + 6;
    if (byClass) drawClassLegend(pdf, state, legendX, legendY);
    else drawTutorLegend(pdf, state, labels, legendX, legendY, pageW - margin - legendX);
  }
