    ...
    for (var day = 0; day < U.DAYS; day++) {
      ...
      dayBlocks.forEach(function (a) {
        ...
        if (bh > 20) {
          ...
          if (room && !roomLine) {
            when = fitText(pdf, U.formatRange(a.startSlot, a.endSlot) + ' · ' + room, textW, [6, 5.5, 5]);
          }
          pdf.text(when, textX, by + 16);
        }
        if (tall) {
          ...
          if (roomLine) {
            ...
            pdf.text(fitText(pdf, room, textW, [6, 5.5, 5]), textX, by + 32);
          }
        }
      });
    }
