...
(function (root) {
  ...
  function testPeriods(r) {
    ...
    st().settings.effective = 'Aug 24 – Oct 9';
    ...
    r.eq(st().settings.effective, '', 'and effective dates of its own');

    // Changing the 2nd leaves the 1st alone.
    TS.store.removeAssignment(st().assignments[0].id);
    st().settings.effective = 'Oct 12 – Dec 10';
    TS.store.commit('remove');
    TS.store.switchPeriod(0);
    TS.store.commit('period');
    r.eq(st().assignments.length, 1, 'the 1st keeps its shift when the 2nd drops it');
    r.eq(st().settings.effective, 'Aug 24 – Oct 9', 'and its own dates');
    TS.store.switchPeriod(1);
    TS.store.commit('period');
    r.eq(st().assignments.length, 0, 'while the 2nd stays as it was left');
    r.eq(U.handoutName(st().settings, new Date(2026, 8, 24)), 'Chemistry Tutoring Schedule 10-12-2026',
      'and its handout is named for its own start date');
    ...
    r.eq(back.periods[0].effective + ' / ' + back.settings.effective, 'Aug 24 – Oct 9 / Oct 12 – Dec 10',
      'and both halves\' dates');
    ...
    // A one-week file -- older, or from the Life Science scheduler -- is the 1st.
    var single = TS.store.migrate({
      settings: { effective: 'Aug 24 – Dec 10' },
      tutors: [{ id: 't', firstName: 'T', availability: [] }],
    ...
    r.eq(single.periods[0].effective, 'Aug 24 – Dec 10', 'keeping its dates');
  ...
  function testHandoutName(r) {
    ...
    var name = function (effective, term) {
      return U.handoutName({ title: 'Chemistry Tutoring Schedule', effective: effective, term: term }, today);
    };
    r.eq(name('Aug 24 – Dec 11', 'Fall 2026'), 'Chemistry Tutoring Schedule 8-24-2026',
      'the file is named for the day the schedule takes effect');
    r.eq(name('August 24, 2026 - December 11, 2026', ''), 'Chemistry Tutoring Schedule 8-24-2026',
      'written out in full, the start date is still found');
    r.eq(name('8/24 - 12/11', 'Spring 2027'), 'Chemistry Tutoring Schedule 8-24-2027',
      'a start with no year takes the semester\'s');
    r.eq(name('2026-08-24 to 2026-12-11', ''), 'Chemistry Tutoring Schedule 8-24-2026',
      'an ISO date reads too');
    r.eq(name('Dec 1 – Jan 15, 2027', ''), 'Chemistry Tutoring Schedule 12-1-2026',
      'dates that run over New Year start in the year before the one written');
    r.eq(name('Starts 8/24/26', ''), 'Chemistry Tutoring Schedule 8-24-2026', 'a two-digit year reads');
    r.eq(name('', 'Fall 2026'), 'Chemistry Tutoring Schedule 9-24-2026',
      'with no effective date, the file is named for today');
    r.eq(name('Feb 30 – Mar 3', ''), 'Chemistry Tutoring Schedule 9-24-2026',
      'and so it is when the start is not a real date');
    r.eq(U.handoutName({ title: 'Gen/Org Chem: Fall', effective: 'Aug 24' }, today),
      'Gen Org Chem Fall 8-24-2026',
